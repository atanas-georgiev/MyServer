...
namespace MyServer.ViewComponents.ImageGallery.Components.LastestAddedImages.Controllers
{
    public class LatestAddedImagesViewComponent : ViewComponent
    {
        ...
        public LatestAddedImagesViewComponent(IImageService imageService, IStringLocalizer<ViewComponentResources> localizer)
        {
        ...
        {
            var images = this.imageService.GetAllReqursive().OrderByDescending(x => x.CreatedOn).Take(18).To<LatestAddedImagesViewModel>().ToList();
